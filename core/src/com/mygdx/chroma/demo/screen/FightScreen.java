...
public class FightScreen extends Screen
{
    ...
    {
	player=new Player(terrain[0][0].body.getPosition().x+50, terrain[0][0].body.getPosition().y+300);
	...
	for(int c=0; c<terrain.length; c++)
	{
	    ...
	    terrain[c][0]=new Terrain(c/10f*Constants.WIDTH, stackHeight, "grass-block-top.png");
	    for(int r=1; r<terrain[0].length; r++)
		terrain[c][r]=new Terrain(c/10f*Constants.WIDTH, stackHeight-34f*r, "grass-block-side.png");
	...
	debugMatrix = batch.getProjectionMatrix().cpy().scale(0.0005f*Constants.PIXELS_PER_METER, 0.0005f*Constants.PIXELS_PER_METER, 0);
	sb.begin();
	sb.draw(backdrop,0,0,Constants.WIDTH,Constants.HEIGHT);
	if(player.dir==Constants.LEFT)
	    	sb.draw(player.curSprite,player.body.getPosition().x+100,player.body.getPosition().y,-100,100);
	else
		sb.draw(player.curSprite,player.body.getPosition().x,player.body.getPosition().y,100,100);
	for(Terrain[] stack : terrain)
	    for(Terrain block : stack)
	    {
		sb.setColor(new Color(0.5f, 0.5f, 0.5f, 1));
		sb.draw(block.image, block.body.getPosition().x, block.body.getPosition().y);
